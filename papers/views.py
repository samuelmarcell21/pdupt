...
def search(request):
    ...
    if request.method == 'POST':
        ...
        # import gensim
        model = gensim.models.ldamodel.LdaModel.load('papers/mallet_18_lda.mdl', mmap='r') 
        ...
        global user_list
        user_list = Papers.objects.filter(id_pub__in=id_artikel).order_by('id_pub')
...
def index(request):
    papers = Papers.objects.all()[:40]
    context = {
        'title': 'Halaman Utama',
        'Papers': papers,
    }
    p = Paginator(papers, 2)
    print(p.num_pages)
    page1 = p.page(1)
    print(page1.object_list)
    return render(request, 'papers/indexx.html', context)

def cobak(request):
    user_list = Papers.objects.all().order_by('id_pub')[:100]
    page = request.GET.get('page', 1)
    ...
    return render(request, 'papers/paper.html', { 'users': users })